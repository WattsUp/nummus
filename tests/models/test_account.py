...
def test_add_transactions(self) -> None:
    ...
    assert acct.opened_on_ord == today_ord
    assert acct.updated_on_ord == today_ord
    ...
    assert acct.opened_on_ord == t_before.date_ord
    assert acct.updated_on_ord == today_ord
...
def test_get_asset_qty(self) -> None:
    ...
    result_qty = acct.get_asset_qty(start, end)
    assert result_qty == target_qty
    result_qty = Account.get_asset_qty_all(s, start, end)
    assert result_qty == {acct.id_: target_qty}
...
def test_get_value(self) -> None:
    ...
    assert r_profit == target_profit
    assert r_assets == target_assets

    r_values, r_profit, r_assets = Account.get_value_all(s, start, end)
    assert len(r_values) == 0
    assert len(r_profit) == 0
    assert len(r_assets) == 0
    assert r_values[acct.id_] == target_values
    assert r_profit[acct.id_] == target_profit
    ...
    assert r_values == [target_values[1]]
    assert r_profit == [target_profit[1]]
    assert r_assets == target_assets

    # Profit on third day should be zero
    r_values, r_profit, r_assets = acct.get_value(start + 2, start + 2)
    assert r_values == [target_values[2]]
    assert r_profit == [0]
    assert r_assets == target_assets
    ...
    target_values = [c + v for c, v in zip(target_values, asset_values, strict=True)]
    target_profit = [c + v for c, v in zip(target_profit, asset_values, strict=True)]
    ...
    r_values, r_profit, r_assets = acct.get_value(start + 1, start + 1)
    assert r_values == [target_values[1]]
    assert r_profit == [t_fund + t0 + asset_values[1]]
    assert r_assets == {assets[0].id_: [asset_values[1]]}

    # Profit on day after of asset trade should be valid too
    r_values, r_profit, r_assets = acct.get_value(start + 2, start + 2)
    assert r_values == [target_values[2]]
    assert r_profit == [0]
    assert r_assets == {assets[0].id_: [asset_values[2]]}

    # Profit on day of asset sell should be valid too
    r_values, r_profit, r_assets = acct.get_value(end - 1, end - 1)
    assert r_values == [target_values[5]]
    assert r_profit == [t1 - q0 * prices[5]]
    assert len(r_assets) == 0
    ...
    assert r_values == [target_values[3]]
    assert r_profit == [0]
    assert r_assets == {assets[0].id_: [asset_values[3]]}

    r_values, r_profit, r_assets = Account.get_value_all(s, today_ord, today_ord)
    assert r_values == {acct.id_: [target_values[3]]}
    assert r_profit == {acct.id_: [0]}
    assert r_assets == {assets[0].id_: [asset_values[3]]}
    ...
    assert r_values == {acct.id_: [target_values[-1]]}
    assert r_profit == {acct.id_: [0]}
    assert r_assets == {}
    ...
    assert r_values == [t0 + t1]
    assert r_profit == [t0 + t1]
    assert r_assets == {}
