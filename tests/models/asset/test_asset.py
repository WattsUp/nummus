...
def test_get_value() -> None:
    ...
    assert r_values == {a.id_: target_values}
    ...
    assert r_values == [v_today.value]

    r_values = Asset.get_value_all(s, today_ord, today_ord)
    assert r_values == {a.id_: [v_today.value]}
...
def test_update_splits() -> None:
    ...
    assert t_split_0.asset_quantity_unadjusted == 1
    assert t_split_1.asset_quantity_unadjusted == 1

    assert t_split_0.asset_quantity == 1
    assert t_split_1.asset_quantity == 1
    ...
    assert t_split_0.asset_quantity == 1 * multiplier
    assert t_split_1.asset_quantity == 1
    ...
    s.add_all((txn_0, t_split_0))
    s.commit()
    assert t_split_0.asset_quantity_unadjusted == qty
...
def test_update_valuations() -> None:
    ...
    r_start, r_end = a.update_valuations(through_today=False)
    s.commit()
    assert r_start == date - datetime.timedelta(days=7)
    assert r_end == date + datetime.timedelta(days=7)
    ...
    split_id = split.id_
    assert split.multiplier == Decimal("1.1")
    ...
    assert split and split.date_ord == last_monday.toordinal()
    ...
    assert v and v.date_ord == last_weekday.toordinal()
